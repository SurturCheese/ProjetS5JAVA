...
public class Ghost implements NPC {
  ...
  public void move() {
    if (
      !(Objects.equals(state, SCARED) && skipTurn) ||
      Objects.equals(state, NORMAL)
    ) {
      if (
      ...
      ) posX = posX - PacManView.TILESIZE; else {
        changeDirection();
      }
    }
    skipTurn = !skipTurn;
  ...
  private void changeDirection() {
    ...
    if (north != 1 && south != 1 && east == 1 && west == 1) {
      if (direction.equals(UP)) moveUp(); else moveDown();
    } else if (north == 1 && south == 1 && east != 1 && west != 1) {
      if (direction.equals(RIGHT)) moveRight(); else moveLeft();
    } else if (north != 1 && south != 1 && east != 1 && west != 1) {
